...
type ScreeningRepository interface {
	...
	// evidence reasons onto engine.reasons. verdict overwrites the stored
	// verdict when non-empty; empty string leaves it untouched.
	SetChecks(ctx context.Context, id string, flags []string, matches []model.BlacklistMatch, risk float64, appendReasons []string, faceMatch *model.FaceMatchResult, verdict model.Verdict) (*model.Screening, error)
...
func (r *mongoScreeningRepo) SetChecks(ctx context.Context, id string, flags []string, matches []model.BlacklistMatch, risk float64, appendReasons []string, faceMatch *model.FaceMatchResult, verdict model.Verdict) (*model.Screening, error) {
	oid, err := bson.ObjectIDFromHex(id)
	if err != nil {
		return nil, apperr.ERRORS.ScreeningNotFound
	}
